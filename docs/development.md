...
otherwise idle machine. `fig_runtime.py` also compares with a GPU: run it once where JAX
finds one and once on a CPU (`JAX_PLATFORMS=cpu`), in either order; each run records its own
timings and draws the figure from all that is recorded.
