...
which is why the rate is fitted to the seeded mode. (c) and (d) The relative total-energy
error of each run in (a) and (b); the grey band is below double-precision round-off.
```
