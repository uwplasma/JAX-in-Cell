...
| box length | {{ relativistic_length_c_over_wpe }} $c/\omega_{pe}$, one wavelength, $k v_0/\omega_{pe} = $ {{ relativistic_k_v0_over_wpe }} |
| grid | {{ relativistic_grid_points }} cells, $\Delta x = $ {{ relativistic_dx_wpe_over_c }} $c/\omega_{pe}$ |
...
same input. (a) Electrostatic energy, with $e^{2\gamma t}$ for the fastest box mode of
each dispersion relation (dashed). (b) Relative change of $\mathcal E_{rel}$ (solid)
and $\mathcal E_{N}$ (dotted). The blue solid line stops when the first electron of
the non-relativistic run reaches $|v| \ge c$, after which $\gamma_p$ is not defined.
(c), (d) Electron phase space at the peak of the electrostatic energy; the shaded
bands are $|v_x| > c$. Generated by `docs/scripts/fig_relativistic.py`.
```
