...
import numpy as np
from common import (COLORS, C_ELECTRONS, C_IONS, C_THEORY, figure, panel_label, record,
                    savefig)
...
def sheath(cells=CELLS, capacity=CAPACITY, emit=EMIT, dt=DT, reflection=0.0):
    ...
    electrons = Species("electrons", capacity, -1.0, mass_electron, DENSITY, (np.sqrt(2) * SIGMA, 0, 0),
                        active=capacity // 4, sampling="quiet", reflection=(0.0, reflection),
...
axes[0].axhline(PHI_WALL, ls="--", lw=2, color=C_THEORY, label="kinetic reference")
...
axes[1].plot(centres, reference_e, "--", lw=2, color=C_THEORY, label=r"$n(\phi)$, kinetic")
axes[1].plot(centres, reference_i, "--", lw=2, color=C_THEORY)
...
record(
    ...
    source_sheath_density_error_electrons=round(float(np.abs(n_e - reference_e)[2:-2].max()), 4),
    source_sheath_density_error_ions=round(float(np.abs(n_i - reference_i)[2:-2].max()), 4),
