...
# --- 1. Staggered grid -----------------------------------------------------
fig, ax = figure(aspect=0.32)
ax.set_xlim(-0.8, 5.8)
ax.set_ylim(-1.0, 1.6)
ax.axis("off")
for i in range(6):
    ax.axvline(i - 0.5, color="#BBBBBB", lw=2, zorder=0)
ax.axhline(0, color="#444444", lw=2)
for i in range(6):
    ax.plot(i, 0, "o", ms=7, color=C_ELECTRONS, zorder=3)
    ax.text(i, -0.42, rf"$x_{{{i}}}$" if i < 5 else r"$x_{N_x-1}$", ha="center", va="top")
    if i < 5:
...
ax.text(2, 0.55, r"cell faces $x_{i+1/2}$: $\mathbf{E}$, $\mathbf{J}$", color=C_IONS,
        ha="center")
ax.annotate("", xy=(1.0, -0.72), xytext=(0.0, -0.72), arrowprops=dict(arrowstyle="<->", lw=2))
ax.text(0.5, -0.8, r"$\Delta x = L / N_x$", ha="center", va="top")
ax.text(-0.42, 1.3, r"$x = -L/2$", ha="left")
ax.text(5.42, 1.3, r"$x = +L/2$", ha="right")
...
fig, ax = figure(aspect=0.41)
ax.set_xlim(-0.9, 2.6)
ax.set_ylim(-0.3, 3.9)
ax.axis("off")
rows = {"x": 3.0, "v": 2.0, "E, B": 1.0, "J": 0.0}
for label, y in rows.items():
    ax.text(-0.85, y, label, ha="left", va="center", fontweight="bold")
    ax.axhline(y, color="#DDDDDD", lw=2, zorder=0)
for tt, label in ((-0.5, r"$n-1/2$"), (0, r"$n$"), (0.5, r"$n+1/2$"), (1.0, r"$n+1$"), (1.5, r"$n+3/2$")):
    ax.axvline(tt, color="#EEEEEE", lw=2, zorder=0)
    ax.text(tt, 3.6, label, ha="center")
...
push = dict(arrowstyle="->", lw=2, color=C_ELECTRONS)
...
ax.text(0.5, rows["v"] + 0.18, r"Boris push with $\mathbf{E}^{n+1/2}, \mathbf{B}^{n+1/2}$ at $x^{n+1/2}$",
        ha="center")
advance = dict(arrowstyle="->", lw=2, color=C_IONS)
...
ax.text(0.25, rows["E, B"] - 0.38, r"$\Delta t/2$: E, then B", ha="center")
ax.text(0.75, rows["E, B"] - 0.38, r"$\Delta t/2$: B, then E", ha="center")
ax.text(0.0, rows["J"] - 0.32, r"$\mathbf{J}^{n}$ from $x^{n-1/2}\to x^{n+1/2}$", ha="center")
ax.text(1.0, rows["J"] - 0.32, r"$\mathbf{J}^{n+1}$ from $x^{n+1/2}\to x^{n+3/2}$", ha="center")
feed = dict(arrowstyle="->", lw=2, color=COLORS["green"], ls="--")
...
fig, axes = figure(2, gridspec_kw={"wspace": 0.3})
ax = axes[0]
ax.plot(xi, S0, color=COLORS["grey"], lw=2, label=r"$S_0$: nearest grid point")
