...
* Precision as a choice: `JAX_ENABLE_X64` is respected, and every example sets it at its
  top, so `JAX_ENABLE_X64=0` runs it in single precision without editing the script. Single
  precision reproduces the examples' rates and frequencies but not conservation to
  round-off, and is not automatically faster ({doc}`../user_guide/units`,
