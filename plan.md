...
| ID | Status | Finding | Required correction |
|---|---|---|---|
| S20 | **open** | The kinetic relation predicts an electron density rising as `exp(phi)` across the presheath hump, 4.4 % at the quick preset's +0.043 T_e/e, and the measured density is flat there to 0.5 %. The ions agree to 0.003 over the same stretch, so it is not the window, the coordinates or the moments. Quasi-neutrality cannot hold with a 4 % electron excess and a flat ion density, so at least one of the two is describing something the other is not. | Candidates, in order: the relation assumes a monotonic drop from the plane, and a hump reflects the slow injected electrons before they arrive, so the population beyond it is not the one the formula counts; the finite box lets wall-reflected electrons leave through the source plane, depleting the returning half; the potential is reconstructed by integrating E from the plane, so a systematic error there tilts the whole profile. Settle it before the sheath benchmark is quoted again (W7), by a run with a deliberately monotonic potential and by an independent reconstruction of phi. |
