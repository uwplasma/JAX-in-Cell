# Electron-only dispersion relation in normalized vars:
#   khat = k * lambda_D,  omegahat = omega / omega_pe
# Newton on R^2; residual & Jacobian JITed; manual 2x2 linear solve.
#
# Run:
#   python solve_dispersion_jax_norm.py --khat 0.3 --n0 1e18 --Te_eV 10 --print-params
...
# ============================
# Faddeeva / Plasma Z function
# Weideman-style partial fractions (pure JAX, no complex erfc)
# ============================
def _weideman_coeffs(M=64, L=6.0):
    m = jnp.arange(-M, M+1)                  # integer nodes
    t = (m * jnp.pi) / L                     # poles (real)
    ccoefs = (1.0 / L) * jnp.exp(-t**2)      # weights
    return t, ccoefs
...
@jit
def wofz_weideman(z):
    # scalar complex z supported; sum c_m/(z - t_m)
...
# ----------------------------
# Electron Landau integral I_e (dimensional)
# I_e = n0/(k vth^2) * [1 + zeta Z(zeta)],  zeta = omega/(k vth)
# ----------------------------
@jit
...
# ----------------------------
# Simplified electron-only residual (dimensional)
# Algebraic factorization to avoid redundant broadcasts:
# F = H + G*(me*Ie) + (G/(eps0*k)) * ((qe^2 * Ie^2) / (1 - (qe^2/(me*eps0*k))*Ie))
# with qe=-e. We factor Den = 1 - (qe^2/(me*eps0*k))*Ie once.
# ----------------------------
@jit
def dispersion_residual_dim(omega, k, n0, vth):
    qe = -e
    Ie = I_e_dim(omega, k, n0, vth)           # complex
    H  = H_dim(omega, k)
    G  = G_dim(omega, k)
    Den = 1.0 - (qe**2 / (me * eps0 * k)) * Ie
    term_em = (G / (eps0 * k)) * ((qe**2) * (Ie**2) / Den)
    return H + G * (me * Ie) + term_em

# ----------------------------
# Normalized residual on R^2:
# khat = k*lambda_D,  omegahat = omega/omega_pe
# ----------------------------
def residual_R2_norm(what_re_im, khat, n0, Te_J):
    omega_pe, vth, lambda_D = plasma_params(n0, Te_J)
    k = khat / lambda_D
    omega = (what_re_im[0] + 1j * what_re_im[1]) * omega_pe
    F = dispersion_residual_dim(omega, k, n0, vth)
    # scale to keep magnitudes tame
...
# Bohm–Gross: omegahat^2 ≈ 1 + (3/2) khat^2
# ----------------------------
...
# ----------------------------
# Newton on R^2 (NOT JIT — avoids TracerBoolConversionError)
# Residual & Jacobian are JITed and fast.
# ----------------------------
def newton_norm(w0_hat, khat, n0, Te_J, max_iter=60, tol=1e-11):
    w = w0_hat
    for _ in range(max_iter):
        r = residual_R2_norm_jit(w, khat, n0, Te_J)   # (2,)
        J = jac_R2_norm(w, khat, n0, Te_J)            # (2,2)
        delta = -solve2x2(J, r)                       # (2,)
        w = w + delta
        if float(jnp.linalg.norm(r, ord=jnp.inf)) < tol:
            return w, True
    return w, False

def solve_for_khat(khat, n0, Te_J):
    w0 = initial_guess_norm(khat)
    w_sol, ok = newton_norm(w0, khat, n0, Te_J)
...
def scan_and_plot(kmin, kmax, npts, n0, Te_J):
    ...
    for i, khat in enumerate(ks):
        ...
        wr[i], wi[i], okv[i] = wseed.real, wseed.imag, bool(ok)
    ...
    plt.title('Growth/damping rate vs normalized wavenumber')
...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="Electron-only dispersion (JAX, normalized).")
    ...
    if args.scan:
        ...
        scan_and_plot(kmin, kmax, npts, n0, Te_J)
        return
    ...
    what, ok = solve_for_khat(khat, n0, Te_J)
