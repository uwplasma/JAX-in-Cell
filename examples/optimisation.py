...
A Simulation is a JAX pytree whose physical parameters are leaves, so derivatives
of any diagnostic with respect to them come straight from JAX: the time loop, the
field solve, the deposition and the particle push are all differentiated, with no
...
With a single parameter the derivative is taken in forward mode, jax.jvp. It costs
about as much as reverse mode, jax.grad, which gives the same number, but it stores
nothing along the run, and it is the mode that works on Apple's Metal backend.
"""
import jax
...
def amplification(drift):
    ...
    # |E_1|, the first Fourier mode, as a projection, which needs no complex arithmetic
    return 0.5 * jnp.log(jnp.sum(E * jnp.cos(phase)) ** 2 + jnp.sum(E * jnp.sin(phase)) ** 2)


value_and_derivative = jax.jit(lambda drift: jax.jvp(amplification, (drift,), (1.0,)))

# the derivative is one forward-mode pass through the whole run; check it once
derivative = float(value_and_derivative(4.0e7)[1])
h = 1e4
difference = float((jax.jit(amplification)(4.0e7 + h) - jax.jit(amplification)(4.0e7 - h)) / (2 * h))
print(f"forward mode {derivative:.6e}   central difference {difference:.6e}   "
      f"relative error {abs(difference / derivative - 1):.1e}\n")

drift, history = 2.5e7, []
for iteration in range(12):
    objective, slope = value_and_derivative(drift)
