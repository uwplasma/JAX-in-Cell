...
__all__ = [
    'fields_to_particles_grid', 'rotation',
    'boris_step', 'boris_step_relativistic',
    'boris_step_with_force', 'boris_step_relativistic_with_force'
]

@jit
def boris_step_with_force(dt, xs_nplushalf, vs_n, q_ms, E_fields_at_x, B_fields_at_x, a_ext):
    """
    Nonrelativistic Boris with an extra acceleration a_ext (N,3).
    """
    vs_n_int = vs_n + (q_ms) * E_fields_at_x * dt / 2 + a_ext * dt / 2
    vs_n_rot = vmap(lambda B_n, v_n, q_m: rotation(dt, B_n, v_n, q_m))(B_fields_at_x, vs_n_int, q_ms[:, 0])
    vs_nplus1 = vs_n_rot + (q_ms) * E_fields_at_x * dt / 2 + a_ext * dt / 2
    xs_nplus3_2 = xs_nplushalf + dt * vs_nplus1
    return xs_nplus3_2, vs_nplus1


@jit
def boris_step_relativistic_with_force(dt, xs_nplushalf, vs_n, q_s, m_s, E_fields_at_x, B_fields_at_x, F_ext):
    """
    Relativistic Boris with an extra force F_ext (N,3).
    """
    def single_particle_step(x, v, q, m, E, B, F):
        gamma_n = 1.0 / jnp.sqrt(jnp.maximum(1.0 - jnp.sum((v / c) ** 2), 1e-30))
        p_n = gamma_n * m * v

        # half-step with E and F_ext
        p_minus = p_n + (q * E + F) * dt / 2

        # magnetic rotation only
        p_plus = relativistic_rotation(dt, B, p_minus, q, m)

        # second half-step
        p_nplus1 = p_plus + (q * E + F) * dt / 2
    ...
    xs_nplus3_2, vs_nplus1 = vmap(single_particle_step)(
        xs_nplushalf, vs_n, q_s, m_s, E_fields_at_x, B_fields_at_x, F_ext
    )
    return xs_nplus3_2, vs_nplus1
...
def boris_step_relativistic(dt, xs_nplushalf, vs_n, q_s, m_s, E_fields_at_x, B_fields_at_x):
    ...
    def single_particle_step(x, v, q, m, E, B):
        ...
        gamma_n = 1/jnp.sqrt(jnp.maximum(1.0 - jnp.sum((v / c) ** 2), 1e-30))
