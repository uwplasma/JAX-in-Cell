...
def tiny_openpmd_output(tmp_path, **solver_overrides):
...
def test_openpmd_output_paths_iterate_without_overwrite(tmp_path):
    ...
    """
    no_extension = tmp_path / "series"
    paths = openpmd_output_paths(str(no_extension), write_pmd_sidecar=False)
    assert paths["data"]["combined"] == str(tmp_path / "series.h5")

    occupied = tmp_path / "occupied.h5"
    occupied.write_text("old data")
    iterated = openpmd_output_paths(str(occupied), write_pmd_sidecar=False)
    assert iterated["data"]["combined"] == str(tmp_path / "occupied0.h5")
    ...
    sidecar_iterated = openpmd_output_paths(
        str(tmp_path / "sidecar_only.h5"),
        write_pmd_sidecar=False,
    )
    assert sidecar_iterated["data"]["combined"] == str(tmp_path / "sidecar_only0.h5")
    assert sidecar_iterated["sidecar"] == {}

    separate_mesh = tmp_path / "split_meshes.h5"
    separate_mesh.write_text("old mesh data")
    separate = openpmd_output_paths(
        str(tmp_path / "split.h5"),
        separate_particles_and_meshes=True,
        write_pmd_sidecar=True,
    )
    assert separate["data"]["meshes"] == str(tmp_path / "split0_meshes.h5")
    assert separate["data"]["particles"] == str(tmp_path / "split0_particles.h5")
...
def test_write_openpmd_combined_series(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setitem(sys.modules, "openpmd_api", fake_openpmd_module())
    ...
    assert paths["data"]["combined"] == str(tmp_path / "jaxincell_output.h5")
    assert (tmp_path / "jaxincell_output.pmd").read_text() == "jaxincell_output.h5\n"
    assert len(FakeSeries.created) == 1

    series = FakeSeries.created[0]
    assert series.path == str(tmp_path / "jaxincell_output.h5")
...
def test_write_openpmd_separate_series_and_bp_extension(monkeypatch, tmp_path):
    """Test jaxincell._openpmd.write_openpmd separate particle and mesh output.

    Cases:
    - .bp filenames are preserved.
    ...
    output = tiny_openpmd_output(
        tmp_path,
        openpmd_filename=str(tmp_path / "split.bp"),
        openpmd_separate_particles_and_meshes=True,
    )

    paths = write_openpmd(output)

    assert paths["data"]["meshes"] == str(tmp_path / "split_meshes.bp")
    assert paths["data"]["particles"] == str(tmp_path / "split_particles.bp")
    assert (tmp_path / "split_meshes.pmd").read_text() == "split_meshes.bp\n"
    assert (tmp_path / "split_particles.pmd").read_text() == "split_particles.bp\n"
    assert [series.path for series in FakeSeries.created] == [
        str(tmp_path / "split_meshes.bp"),
        str(tmp_path / "split_particles.bp"),
    ]
...
def test_simulation_run_writes_openpmd_when_enabled(monkeypatch, tmp_path):
    ...
    def fake_writer(output):
        ...
        return {"data": {"combined": output["solver_parameters"]["openpmd_filename"]}, "sidecar": {}}
