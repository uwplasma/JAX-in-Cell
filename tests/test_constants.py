...
from jaxincell._constants import (
    epsilon_0, mu_0, speed_of_light, charge_electron, charge_proton,
    mass_electron, mass_proton, q_m_electron, q_m_proton
)
...
def test_charge_electron():
    assert charge_electron == -1.60217663e-19, "Incorrect value for charge_electron"

def test_charge_proton():
    assert charge_proton == 1.60217663e-19, "Incorrect value for charge_proton"
...
def test_q_m_electron():
    expected_value = charge_electron / mass_electron
    assert q_m_electron == expected_value, "Incorrect value for q_m_electron"

def test_q_m_proton():
    expected_value = charge_proton / mass_proton
